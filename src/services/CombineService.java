...
public class CombineService {
    ...
    public void doThemDong(@NonNull Player player, short indexItem, short idMaterial, boolean isLockMaterial, short idDaTienGiai, boolean isLockDaTienGiai) throws IOException {
        ...
        if (itemBag.getColorName() != ItemEquipConst.GREEN_COLOR || itemBag.getRank() != ItemEquipConst.NHAT_PHAM || itemBag.getLevel() < 50) {
            Service.instance.sendLogOut(player.getSession(), "Cần trang bị nhất phẩm hoàn mỹ level 50 trở lên - [Khóa]");
            return;
        }
        if (itemBag.getItemAttributes().stream().filter(i -> i != null && i.getTemplate().getColorPaint() == 1).count() >= 2) {
            Service.instance.sendLogOut(player.getSession(), "Trang bị đã thêm tối đa dòng");
            return;
        }
        ItemGem botXanhLa = isLockMaterial ? InventoryService.instance.findItemGemLock(player, idMaterial) : InventoryService.instance.findItemGem(player, idMaterial);
        ...
        ItemGem daTienGiai = isLockDaTienGiai ? InventoryService.instance.findItemGemLock(player, idDaTienGiai) : InventoryService.instance.findItemGem(player, idDaTienGiai);
        ...
        if (Util.isTrue(2.6, 200.0)) {
            Attribute dongAn1 = itemBag.getItemAttributes().stream().filter(att -> att != null && att.getTemplate().getColorPaint() == 1).findFirst().orElse(null);
            short valueAn = 1;
            short idAttribute;
            do {
                idAttribute = ItemEquipConst.ATTRIBUTE_RANDOM_DONG_AN[Util.nextInt(0, ItemEquipConst.ATTRIBUTE_RANDOM_DONG_AN.length - 1)];
            ...
            Service.instance.sendLogOut(player.getSession(), String.format("Chúc mừng bạn đã mở được %s %s%s", attributeAdd.getTemplate().getName(), valueAn, "%"));
        } else {
            if (itemBag.isWeapon() || itemBag.isJewelry()) {
                Attribute attribute = itemBag.getItemAttributes().stream().filter(att -> att != null && Util.binarySearch(ItemEquipConst.ATTRIBUTE_RANDOM_TIEN_GIAI_VU_KHI, att.getTemplate().getId())).findFirst().orElse(null);
                if (attribute != null) {
                    short attributeId;
                    do {
                        attributeId = ItemEquipConst.ATTRIBUTE_RANDOM_TIEN_GIAI_VU_KHI[Util.nextInt(0, ItemEquipConst.ATTRIBUTE_RANDOM_TIEN_GIAI_VU_KHI.length - 1)];
                    ...
                    itemBag.getItemAttributes().add(attributeAdd);
                    Service.instance.sendLogOut(player.getSession(), String.format("Chúc mừng bạn đã mở được %s", attributeAdd.getInfo()));
                } else {
                    short attributeId = ItemEquipConst.ATTRIBUTE_RANDOM_TIEN_GIAI_VU_KHI[Util.nextInt(0, ItemEquipConst.ATTRIBUTE_RANDOM_TIEN_GIAI_VU_KHI.length - 1)];
                    short value = 0;
                    if (attributeId == 109) {
                ...
                Attribute attribute = itemBag.getItemAttributes().stream().filter(att -> att != null && Util.binarySearch(ItemEquipConst.ATTRIBUTE_RANDOM_TIEN_GIAI, att.getTemplate().getId())).findFirst().orElse(null);
                if (attribute != null) {
                    byte quantityAttributeNew = (byte) Util.nextInt(1, (int) itemBag.getItemAttributes().stream().filter(att -> att != null && Util.binarySearch(ItemEquipConst.ATTRIBUTE_RANDOM_TIEN_GIAI, att.getTemplate().getId())).count());
                    StringBuilder builder = new StringBuilder();
                    for (int i = itemBag.getItemAttributes().size() - quantityAttributeNew; i < itemBag.getItemAttributes().size(); i++) {
                    ...
                    Service.instance.sendLogOut(player.getSession(), String.format("Chúc mừng bạn đã mở được %s", builder.toString()));
                } else {
    ...
    public void doTachNguyenLieu(@NonNull Player player, byte type, short indexItem, short idMaterial, boolean isLock) throws IOException {
        ItemEquip itemBag = InventoryService.instance.findItemBag(player, indexItem);
        if (itemBag == null || itemBag.getTemplate().getNdayLoan() != 0) {
            return;
        }
        switch (type) {
            case CombineConst.NGUYEN_BOT -> {
                if (!Util.isNullOrEmpty(itemBag.getNameCharSeal()) || itemBag.getColorName() == ItemEquipConst.NONE_COLOR) {
                    Service.instance.sendLogOut(player.getSession(), "Chỉ có thể nghiền trang bị rơi từ quái hoặc trang bị màu");
                    return;
                }
